...
как к коллекции даёт `keySet()`, `values()` и `entrySet()`.

### 2. Чем ключ отличается от значения?

**Ответ:** ключ уникален, участвует в вычислении бакета и требует корректных
`equals` и `hashCode`. Значения могут повторяться, и никаких требований к ним нет.

### 3. Что возвращает put()?

**Ответ:** предыдущее значение по этому ключу либо `null`, если ключа не было.
Стоит помнить, что `null` может означать и отсутствие ключа, и хранившееся
`null`-значение — различить помогает `containsKey()`.

### 4. Чем get() отличается от getOrDefault()?

**Ответ:** `get()` возвращает `null` при отсутствии ключа, `getOrDefault()` —
указанное значение по умолчанию. Второй вариант избавляет от проверки на `null`.
...
связку «проверить, создать, положить» одной атомарной по смыслу операцией. Типичный
случай — карта списков.

### 8. Чем compute() отличается от computeIfPresent()?

**Ответ:** `compute()` вызывается независимо от наличия ключа и получает `null` при
его отсутствии. `computeIfPresent()` срабатывает только для существующего ключа.
Возврат `null` из функции в обоих случаях удаляет запись.
...
**Ответ:** определяет бакет. От него зависит скорость: плохой хеш собирает все ключи
в одном бакете и превращает поиск в перебор.

### 12. Какую роль выполняет equals()?

**Ответ:** находит точный ключ внутри бакета. Совпадение хешей не означает
равенства ключей.
...
логарифмическим вместо линейного. При уменьшении числа элементов дерево
разворачивается обратно.

### 15. Что такое capacity?

**Ответ:** текущее число бакетов во внутреннем массиве. Всегда является степенью
двойки, что позволяет вычислять индекс битовой операцией вместо деления.

### 16. Что такое load factor?

**Ответ:** коэффициент заполнения, по умолчанию 0,75. Определяет, при какой доле
занятости таблица расширяется. Значение выбрано как компромисс между расходом
памяти и вероятностью коллизий.

### 17. Что такое threshold?

**Ответ:** произведение capacity на load factor — число записей, после которого
запускается расширение.
...
вовсе. Отсюда следствие: несогласованность `compareTo` и `equals` приводит к
...
**Ответ:** по умолчанию, когда порядок не важен и нужен быстрый доступ по ключу.

### 30. Когда следует выбирать LinkedHashMap?

**Ответ:** когда требуется предсказуемый порядок обхода — вставки или доступа, — а
также для реализации LRU-кеша.

### 31. Когда следует выбирать TreeMap?

**Ответ:** когда ключи должны храниться отсортированными или нужны диапазонные и
